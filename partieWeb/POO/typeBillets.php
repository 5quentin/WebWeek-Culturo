...
class typeBillets
{
    ...
    public function SuprimerTypeBillet($id_Billet){
        $BDD = new ConnexionBDD();

        $this->reqpreparee = $BDD->connection -> prepare("DELETE FROM `type_billet` WHERE id=".$id_Billet.";");

        $req = $this->reqpreparee->execute();
        if($req==true){
            echo "<script>alert('Type billet Sup')</script>";
        }
    }
}
